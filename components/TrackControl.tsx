...
export const TrackControl: React.FC<TrackControlProps> = ({
  ...
  const height = track.showAutomation
    ? TRACK_HEIGHT + AUTOMATION_HEIGHT
    : TRACK_HEIGHT;
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-1.5">
        ...
        <div className="flex items-center gap-2">
          {!isMaster && showInputChannelSelector && (
            <select
      ...
      <div className="flex items-center gap-1 mb-1.5 relative h-6">
        <div
          className={`flex items-center gap-1 flex-1 transition-opacity duration-200 ${isSliderHovered ? "opacity-0 pointer-events-none" : "opacity-100"}`}
        >
          {!isMaster && (
            <>
              {/* Arm Button */}
                ...
                className={`p-1 rounded-none transition-none ${
                  isArmed
                    ? isRecordingGlobal
                      ...
                      : "bg-[#ef4444]/30 text-[#ef4444] border border-[#ef4444]"
                    : "bg-[#444] text-[#999] hover:text-[#d4d4d4] hover:bg-[#555]"
                }`}
                title={
                  isArmed
                    ? "Disarm Track (Recording Input)"
                    : "Arm Track for Recording"
                }
              >
                <div
                  className={`w-2.5 h-2.5 rounded-full ${isArmed ? "bg-current" : "bg-current"}`}
                />
              </button>
          ...
          {isMaster && (
            <>
              ...
              <div className="flex items-center ml-1">
                <span className="text-[9px] text-[#999] uppercase font-bold mr-1">
                  Vol
                </span>
                <input
                  type="number"
                  value={
                    track.volume <= 0
                      ? -60
                      : Math.max(-60, 20 * Math.log10(track.volume)).toFixed(1)
                  }
                  onChange={(e) => {
        ...
        <div
          className={`flex items-center transition-all duration-300 z-10 bg-[#2d2d2d] ${isSliderHovered ? "absolute left-0 right-0 h-full px-2 shadow-lg border border-[#333] rounded-sm gap-2" : "flex-[0.8] ml-1"}`}
          onClick={(e) => e.stopPropagation()}
        ...
        >
          <input
            ...
            value={
              track.volume <= 0
                ? -60
                : Math.max(-60, 20 * Math.log10(track.volume))
            }
            onChange={(e) => {
              const db = parseFloat(e.target.value);
              const linear = db <= -60 ? 0 : Math.pow(10, db / 20);
            ...
            disabled={
              track.showAutomation && track.selectedAutomationId === "volume"
            }
            title={`Volume: ${track.volume <= 0 ? "-inf" : Math.max(-60, 20 * Math.log10(track.volume)).toFixed(1)} dB (Double-click to reset)`}
          />
          {isSliderHovered && (
            <span className="text-[10px] text-[#ff7b00] font-mono font-bold flex-shrink-0 w-10 text-right whitespace-nowrap">
              {track.volume <= 0
                ? "-inf"
                : Math.max(-60, 20 * Math.log10(track.volume)).toFixed(1)}{" "}
              dB
      ...
      <div
        className="h-1.5 w-full bg-[#111] rounded-none overflow-hidden mt-0.5 relative border border-[#111]"
      ...
      >
        ...
        <div className="absolute inset-0 flex justify-between px-2 pointer-events-none opacity-50">
          <div
            className="w-px h-full bg-black/50"
            style={{ left: "50%" }}
          ></div>
          <div
            className="w-px h-full bg-black/50"
            ...
            <div className="relative" title="Select Parameter to Automate">
              ...
              >
                ...
                {track.plugins.map((p) => {
                  ...
                  if (p.type === "DELAY")
                    return (
                      <option key={p.id} value={`${p.id}:time`}>
                        Delay Time
                      </option>
                    );
                  if (p.type === "DISTORTION")
                    return (
                      <option key={p.id} value={`${p.id}:drive`}>
                        Distortion
                      </option>
                    );
                  if (p.type === "FILTER")
                    return (
                      <option key={p.id} value={`${p.id}:frequency`}>
                        Filter Freq
                      </option>
                    );
                  return null;
