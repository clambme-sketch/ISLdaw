...
const TimeDisplay: React.FC<{
  ...
  handleTimerContextMenu: (e: React.MouseEvent) => void;
...
export const Transport: React.FC<TransportProps> = ({
  ...
  const [appName, setAppName] = useState(
    () => localStorage.getItem("applet_appName") || "ISLdaw",
  );
  const [isEditingName, setIsEditingName] = useState(false);
  const [tempName, setTempName] = useState(appName);

  useEffect(() => {
    localStorage.setItem("applet_appName", appName);
  }, [appName]);
  ...
  useEffect(() => {
    ...
    if (showCountInMenu || showTimerMenu)
    ...
    <div className="h-14 bg-[#2d2d2d] border-b border-[#111] flex items-center z-30 shadow-none select-none overflow-x-auto custom-scrollbar-hidden">
      <div className="flex items-center justify-between px-4 min-w-max w-full gap-4">
        <div className="flex items-center gap-4">
          {isEditingName ? (
            <input
              type="text"
              autoFocus
              className="text-xl font-bold bg-[#1e1e1e] text-[#d4d4d4] mr-2 flex-shrink-0 tracking-tight border border-[#ff7b00] outline-none px-1 py-0.5 rounded-sm w-32"
              value={tempName}
            ...
            <div
              className="text-xl font-bold text-[#d4d4d4] mr-2 flex-shrink-0 tracking-tight cursor-text select-text hover:text-white transition-colors"
          ...
          <div className="flex items-center bg-[#1e1e1e] rounded-md border border-[#333] overflow-hidden flex-shrink-0">
            ...
            >
              <div
                className={`w-3 h-3 rounded-full ${isRecording ? "bg-white" : "bg-current"}`}
              />
            </button>
            <button
        ...
        <div className="flex items-center gap-4">
          {/* BPM & Metronome */}
          <div
            className="flex items-center bg-[#1e1e1e] rounded-md border border-[#333] overflow-hidden group focus-within:border-[#555] flex-shrink-0"
          ...
          >
            <div className="relative">
              ...
              {showCountInMenu && (
                <div className="absolute top-full left-0 mt-1 bg-[#2d2d2d] border border-[#111] rounded-none shadow-none z-50 w-32 py-1 text-xs">
                  <div className="px-2 py-1 text-[#999] font-bold uppercase border-b border-[#111] mb-1">
                    Count In
                  </div>
            ...
            <div className="flex flex-col items-start px-2 py-1 justify-center">
              <span className="text-[8px] text-[#999] font-bold leading-none uppercase">
                BPM
              </span>
              <input
                type="number"
                value={bpm}
                onChange={(e) =>
                  setBpm(Math.max(1, Math.min(999, Number(e.target.value))))
                }
          ...
          <div className="flex items-center bg-[#1e1e1e] rounded-md border border-[#333] overflow-hidden flex-shrink-0">
            <button
              ...
              className={`p-1.5 ${snap ? "text-black bg-[#ff7b00]" : "text-[#999] hover:text-[#d4d4d4] hover:bg-[#333]"}`}
              title="Snap to Grid (Quantize)"
            >
              <Grid size={14} />
            </button>
            <div className="w-px h-4 bg-[#333] mx-0"></div>
      ...
      {showTimerMenu && (
        ...
        >
          <div className="px-3 py-1 text-[10px] font-bold text-[#999] uppercase border-b border-[#111] mb-1">
            Display Options
          </div>
          ...
          >
            ...
            {timeDisplayFormat === "TIME" && (
              <Check size={12} className="text-[#ff7b00]" />
            )}
          </button>
