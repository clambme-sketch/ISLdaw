...
export const Timeline: React.FC<TimelineProps> = ({
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("playhead-update", handleTimeUpdate);
  }, [zoom]);
  ...
  const [resizeStartValues, setResizeStartValues] = useState<{
    startTime: number;
    duration: number;
    offset: number;
  }>({ startTime: 0, duration: 0, offset: 0 });
  ...
  // Loop Dragging State
  const [loopDragType, setLoopDragType] = useState<
    "START" | "END" | "MOVE" | null
  >(null);
  const [loopDragStartX, setLoopDragStartX] = useState<number>(0);
  const [loopDragStartValues, setLoopDragStartValues] = useState<{
    start: number;
    end: number;
  }>({ start: 0, end: 0 });
  ...
  const importTargetRef = useRef<{ trackId: string; time: number } | null>(
    null,
  );
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (rulerDrag && setZoom && timelineRef.current) {
        ...
        const newZoom = Math.max(
          10,
          Math.min(500, rulerDrag.initialZoom * factor),
        );
        setZoom(newZoom);
      ...
      if (marquee?.active) {
        ...
        setMarquee((prev) =>
          prev ? { ...prev, currentX, currentY: relativeY } : null,
        );
        ...
        const clipElements =
          timelineRef.current.querySelectorAll("[data-clip-id]");

        clipElements.forEach((el) => {
          const clipRect = el.getBoundingClientRect();
          const clipRelX =
            clipRect.left - rect.left + timelineRef.current!.scrollLeft;
          const clipRelY =
            clipRect.top - rect.top + timelineRef.current!.scrollTop;

          if (
            mX < clipRelX + clipRect.width &&
            mX + mW > clipRelX &&
      ...
      if (loopDragType) {
        ...
        if (loopDragType === "START") {
          newStart = Math.min(
            newEnd - 0.25,
            Math.max(0, loopDragStartValues.start + diffSeconds),
          );
        } else if (loopDragType === "END") {
          newEnd = Math.max(
            newStart + 0.25,
            loopDragStartValues.end + diffSeconds,
          );
      ...
      if (resizingClipId && resizeEdge) {
        ...
        if (resizeEdge === "END") {
          ...
          newDuration = Math.max(
            0.05,
            resizeStartValues.duration + diffSeconds,
          );
          if (snap)
            newDuration =
              snapToGrid(resizeStartValues.startTime + newDuration) -
              resizeStartValues.startTime;
          ...
          const nextClips = clips.filter(
            (c) =>
              ...
              c.startTime >=
                resizeStartValues.startTime +
                  resizeStartValues.duration -
                  0.001,
          );
          if (nextClips.length > 0) {
            const nextClipStart = Math.min(
              ...nextClips.map((c) => c.startTime),
            );
          ...
          if (snap) {
            const snappedNewStart = snapToGrid(
              resizeStartValues.startTime + diffSeconds,
            );
            desiredDelta = snappedNewStart - resizeStartValues.startTime;
          }

          // Clamp to previous clip's end
          const prevClips = clips.filter(
            (c) =>
              c.trackId === clip.trackId &&
              c.id !== clip.id &&
              c.startTime + c.duration <= resizeStartValues.startTime + 0.001,
          );
          if (prevClips.length > 0) {
            const prevClipEnd = Math.max(
              ...prevClips.map((c) => c.startTime + c.duration),
            );
          ...
          newOffset =
            resizeStartValues.offset + desiredDelta * clip.playbackRate;
        }
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    if (
      ...
      !target.closest(".loop-region-handle") &&
      !target.closest("[data-automation-track-id]") &&
  ...
  const handleDrop = (e: React.DragEvent, trackId: string) => {
    ...
    if (draggedClipId) {
      ...
      const movingIds = selectedClipIds.includes(draggedClipId)
        ? selectedClipIds
        : [draggedClipId];
      ...
      const snappedNewStartTime = snap
        ? snapToGrid(rawNewStartTime)
        : rawNewStartTime;

      // 3. Calculate the delta applied to all other selected clips
      const timeDelta =
        Math.max(0, snappedNewStartTime) - draggedClip.startTime;
      ...
      for (const update of updates) {
        ...
        const otherClips = clips.filter(
          (c) => c.trackId === targetTrackId && !movingIds.includes(c.id),
        );
        ...
        for (const other of otherClips) {
          ...
          if (
            newStartTime < otherEnd - 0.001 &&
            newEndTime > otherStart + 0.001
          ) {
            hasOverlap = true;
            break;
          }
  ...
  const handleClipContextMenu = (e: React.MouseEvent, clipId: string) => {
    ...
    const rect = timelineRef.current?.getBoundingClientRect();
    const clickX =
      e.clientX - (rect?.left || 0) + timelineRef.current!.scrollLeft;
    const time = Math.max(0, clickX / zoom);
  ...
  const handleMenuAction = (e: React.MouseEvent, action: string) => {
    ...
    if (contextMenu.type === "CLIP") {
      const targetIsSelected =
        contextMenu.targetId && selectedClipIds.includes(contextMenu.targetId);
      const idsToActOn = targetIsSelected
        ? selectedClipIds
      ...
      switch (action) {
        case "delete":
          onDeleteClips(idsToActOn);
          break;
        case "duplicate":
          onDuplicateClips(idsToActOn);
          break;
        case "split":
        ...
        case "rename":
          ...
          break;
        case "copy":
          onCopyClips(idsToActOn);
          break;
        case "loop":
          idsToActOn.forEach((id) => onLoopClip(id));
          break;
        case "flatten":
          if (idsToActOn.length === 1 && onFlattenClip)
            onFlattenClip(idsToActOn[0]);
          else if (idsToActOn.length > 1)
            alert("Flattening multiple clips not supported yet.");
          break;
        case "auto-align":
  ...
  const renderClipContent = (clip: AudioClip, track: Track) => {
    ...
    const loopsNeeded = clip.loop
      ? Math.ceil(totalVisibleTime / singleLoopDuration)
      : 1;
    ...
    return (
      <div
        className={`relative w-full h-full overflow-hidden ${isMuted ? "opacity-40 grayscale" : ""}`}
      >
        <div
          ...
          {Array.from({ length: loopsNeeded }).map((_, i) => {
            ...
            return (
              ...
              >
                <Waveform
                  clip={clip}
                  width={singleLoopWidth}
                  height={TRACK_HEIGHT - 10}
                  color={i === 0 ? "#111111" : "#111111aa"}
                />
              </div>
            );
          })}
      ...
      >
        ...
        >
          {barsArray.map((_, i) => (
            ...
            >
              <span className="absolute top-1 left-1 text-[9px] text-[#999] font-mono select-none">
                {i + 1}
              </span>
          ...
          {markers.map((m, i) => (
            <div
              key={i}
              className="absolute top-0 h-full border-l border-[#ff7b00] group z-20"
          ...
          {loopRegion.enabled && (
            <div
              ...
              style={{
                left: loopRegion.start * zoom,
                width: (loopRegion.end - loopRegion.start) * zoom,
              }}
        ...
        <div
          ...
          style={{ minWidth: `${totalBars * pixelsPerBar}px` }}
        >
                    ...
                    .map((clip) => {
                      ...
                      return (
                        <div
                          ...
                          onClick={(e) => {
                            ...
                            if (tool === "BLADE") {
                              ...
                              if (tool === "MOVE" && !isSelected && !e.shiftKey)
                                setSelectedClipIds([clip.id]);
                            }
                          }}
                          onDoubleClick={(e) => {
                        ...
                        >
                          ...
                          ></div>
                          {clip.loop && (
                            <div className="absolute top-0 right-4 p-1 text-white/70 z-30">
                              <RotateCw size={10} />
                            </div>
                          )}
                          ...
                          {tool === "MOVE" && (
                            <>
                              {/* Left Handle */}
                              <div
                                className="absolute top-0 bottom-0 left-0 w-6 cursor-col-resize hover:bg-white/30 group-hover:bg-black/10 z-40 flex items-center justify-center resize-handle group/handle transition-colors"
                                onMouseDown={(e) =>
                                  handleResizeStart(e, clip, "START")
                                }
                              >
                                <GripVertical
                                  size={14}
                                  className="text-white/0 group-hover:text-white/30 group-hover/handle:text-white/80 transition-colors"
                                />
                              </div>
                              {/* Right Handle */}
                              <div
                                className="absolute top-0 bottom-0 right-0 w-6 cursor-col-resize hover:bg-white/30 group-hover:bg-black/10 z-40 flex items-center justify-center resize-handle group/handle transition-colors"
                                onMouseDown={(e) =>
                                  handleResizeStart(e, clip, "END")
                                }
                              >
                                <GripVertical
                                  size={14}
          ...
          <div className="h-64 flex items-center justify-center text-[#444] select-none pointer-events-none">
            End of tracks
          </div>
        </div>
            ...
            {contextMenu.type === "CLIP" ? (
              <>
                {selectedClipIds.length === 2 && (
                  <>
                    <button
                      onClick={(e) => handleMenuAction(e, "auto-align")}
                      className="w-full text-left px-4 py-2 hover:bg-[#444] flex items-center gap-2 text-[#ff7b00]"
                    >
                      <ArrowLeftRight size={14} /> Auto-Align Phase
                    </button>
                    <div className="h-px bg-[#111] my-1" />
                  </>
                )}
                <button
                  onClick={(e) => handleMenuAction(e, "split")}
                  className="w-full text-left px-4 py-2 hover:bg-[#444] flex items-center gap-2"
                >
                ...
                <button
                  onClick={(e) => handleMenuAction(e, "loop")}
                  className="w-full text-left px-4 py-2 hover:bg-[#444] flex items-center gap-2"
                >
                  <RotateCw size={14} /> Toggle Loop
                </button>
                <button
                ...
                <div className="h-px bg-[#111] my-1" />
                <button
                  onClick={(e) => handleMenuAction(e, "flatten")}
                  className="w-full text-left px-4 py-2 hover:bg-[#444] flex items-center gap-2 text-[#ff7b00]"
                >
                  <FileStack size={14} /> Flatten Clip
                </button>
                <div className="h-px bg-[#111] my-1" />
                <button
                  onClick={(e) => handleMenuAction(e, "delete")}
                  className="w-full text-left px-4 py-2 hover:bg-[#ef4444]/20 text-[#ef4444] flex items-center gap-2"
                >
              ...
              <>
                <button
                  onClick={(e) => handleMenuAction(e, "paste")}
                  disabled={!canPaste}
                  className={`w-full text-left px-4 py-2 flex items-center gap-2 ${canPaste ? "hover:bg-[#444]" : "opacity-50 cursor-not-allowed"}`}
                >
                  <ClipboardPaste size={14} /> Paste
                </button>
                <div className="h-px bg-[#111] my-1" />
                <button
                  onClick={(e) => handleMenuAction(e, "import-file")}
                  className="w-full text-left px-4 py-2 hover:bg-[#444] flex items-center gap-2"
                >
                  <Upload size={14} className="text-[#ff7b00]" /> Import Audio
                  File
                </button>
              </>
            ) : (
              <>
                <div className="px-4 py-2 text-xs font-bold text-[#999] uppercase">
                  Marker Options
                </div>
                <button
                  onClick={(e) => handleMenuAction(e, "add-marker")}
                  className="w-full text-left px-4 py-2 hover:bg-[#444] flex items-center gap-2"
                >
                  <Flag size={14} className="text-[#ff7b00]" /> Place Marker
                </button>
              </>
            )}
          </div>
