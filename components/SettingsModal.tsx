...
import { X, Mic, Speaker, Layers, Timer, Download, Settings, Loader2, Activity, Palette } from 'lucide-react';
...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, onExportStems }) => {
  ...
  const { accentColor, setAccentColor, backgroundColor, setBackgroundColor, fontSize, setFontSize, fontFamily, setFontFamily } = useTheme();
        ...
        <div className="flex flex-1 overflow-hidden">
            ...
            <div className="flex-1 p-6 overflow-y-auto bg-[#2d2d2d]">
                ...
                {activeTab === 'AUDIO' && (
                    <div className="space-y-6 transition-none">
                        <div>
                            ...
                            <p className="mt-2 text-xs text-[#999]">Requires microphone permissions to list devices.</p>
                        </div>
                        ...
                        <div>
                            ...
                            <p className="mt-2 text-xs text-[#999]">Output routing is currently supported on Chrome and Edge.</p>
                        </div>
                    </div>
                )}
                ...
                {activeTab === 'EXPORT' && (
                    <div className="space-y-6 transition-none">
                        <div className="p-4 bg-[#1e1e1e] rounded-none border border-[#111]">
                             <h3 className="text-[#d4d4d4] font-medium mb-2">Export Stems</h3>
                             <p className="text-sm text-[#999] mb-4">Export each track as a separate WAV file. Useful for mixing in other software.</p>
                             <button 
                        ...
                        <div className="p-4 bg-[#1e1e1e] rounded-none border border-[#111] opacity-60">
                             <h3 className="text-[#d4d4d4] font-medium mb-2">Mixdown Settings</h3>
                             <p className="text-sm text-[#999]">Current Format: WAV 16-bit 44.1kHz (Fixed)</p>
                        </div>
                    </div>
                )}
                            ...
                            {calibrationStep === 1 && (
                                <div className="flex flex-col items-center gap-4 text-center">
                                    <div className="flex gap-2 mb-2">
                                        <Activity className="animate-pulse text-[#ff7b00]" size={32} />
                                    </div>
                                    <span className="font-mono text-lg font-bold text-[#ffaa00]">Clap along to the beat (8 clicks)...</span>
                                    <div className="text-xs text-[#999]">We'll average your timing for better accuracy.</div>
                                    <Loader2 className="animate-spin text-[#999] mt-2" size={20} />
                                </div>
                            )}

                            {calibrationStep === 2 && (
                                <div className="flex flex-col items-center gap-2 transition-none">
                                     <div className="text-3xl font-bold text-[#10b981]">{latencyResult.toFixed(1)} ms</div>
                                     <span className="text-sm text-[#999]">Average Latency ({detectedClaps} samples)</span>
                                     <button onClick={startCalibration} className="text-xs text-[#ff7b00] hover:underline mt-2">Retest</button>
                                </div>
                            )}
                            
                            {calibrationError && (
                                <div className="text-[#ef4444] text-sm font-bold bg-[#ef4444]/20 px-4 py-2 rounded-none">{calibrationError}</div>
                            )}
                        </div>

                        <div className="flex flex-col gap-2">
                            <label className="text-sm font-medium text-[#d4d4d4]">Manual Offset (ms)</label>
                            ...
                            <p className="text-[10px] text-[#999]">Higher values shift the recorded clip to the left (earlier in time).</p>
                        </div>
                    </div>
                )}
                ...
                {activeTab === 'APPEARANCE' && (
                    <div className="space-y-6 transition-none">
                        <div>
                            <label className="block text-sm font-medium text-[#d4d4d4] mb-2">Accent Color</label>
                        ...
                        <div>
                            <label className="block text-sm font-medium text-[#d4d4d4] mb-2">Background Color</label>
                            <div className="flex items-center gap-4">
                                <input 
                                    type="color" 
                        ...
                        <div>
                            <label className="block text-sm font-medium text-[#d4d4d4] mb-2">Base Font Size (px)</label>
                            <div className="flex items-center gap-4">
                                <input 
                                    type="range" 
                                    min="12" 
                            ...
                            <p className="mt-2 text-xs text-[#999]">Adjusts the overall scale of the UI typography.</p>
                        </div>

                        <div>
                            <label className="block text-sm font-medium text-[#d4d4d4] mb-2">Font Family</label>
                            <select 
                                value={fontFamily}
                                onChange={(e) => setFontFamily(e.target.value)}
                                className="w-full bg-[#111] border border-[#444] text-[#d4d4d4] text-sm rounded-none p-2.5 outline-none focus:border-[#ff7b00]"
                            >
                                <option value='-apple-system, BlinkMacSystemFont, "SF Pro Text", "Segoe UI", Roboto, Helvetica, Arial, sans-serif'>Apple System (SF Pro)</option>
